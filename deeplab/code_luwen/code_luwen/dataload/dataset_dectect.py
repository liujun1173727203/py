...
from dataload.make_mask import make_mask
...
class MVTecDataset(Dataset):
    def __init__(self,
        ...
        # train_transform.transforms.append(transforms.Resize((resize, resize)))
        # train_transform.transforms.append(CutPasteUnion(transform=after_cutpaste_transform))
        # # train_transform.transforms.append(transforms.ToTensor())

        # set transforms 原来的预处理
        ...
        # self.transform_x = transforms.Compose([
        #     transforms.Resize(resize, Image.ANTIALIAS),
        #     make_mask(cutout_size = random.choice([2,4,8,16]),num_disjoint_masks=4,transform=after_cutpaste_transform)
        # ])
        self.transform_x = transforms.Compose([
            transforms.Resize(resize, Image.ANTIALIAS),
            make_mask(cutout_size=16, num_disjoint_masks=4,
                      transform=after_cutpaste_transform)
        ])
        self.transform_test_x = transforms.Compose(
            [transforms.Resize(resize, Image.NEAREST),
             transforms.ToTensor()])
    ...
    def load_dataset_folder(self):
        ...
        if self.is_val:
            random.shuffle(x)#shuffle函数是没有返回值的

            x = x[:int(len(x) * 0.2)]
            y = y[:int(len(y) * 0.2)]
            mask = mask[:int(len(mask) * 0.2)]
