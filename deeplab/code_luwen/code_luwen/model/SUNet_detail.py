...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.num_patches = patches_resolution[0] * patches_resolution[1]#patch的数量56*56

        self.in_chans = in_chans#c=3
        self.embed_dim = embed_dim#96

        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)#分块操作(B,c,224,224)->(B,embed_dim,56,56)
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)  # B Ph*Pw C （B，56*56, 96）
    ...
    def flops(self):
        ...
        flops = Ho * Wo * self.embed_dim * self.in_chans * (self.patch_size[0] * self.patch_size[1])#56*56*96*3*4*4
        if self.norm is not None:
...
class SUNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=3,
        ...
        self.embed_dim = embed_dim#96
        self.ape = ape#Fasle
        self.patch_norm = patch_norm#Ture
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 1))#96*2**3
        self.num_features_up = int(embed_dim * 2)#96*2
        self.mlp_ratio = mlp_ratio#4
        self.final_upsample = final_upsample#Dual up-sample
        self.prelu = nn.PReLU()#激活函数
        self.conv_first = nn.Conv2d(in_chans, embed_dim, 3, 1, 1)#3->96  维度没变只是输入的通道数发生了改变
        ...
        for i_layer in range(self.num_layers):#4
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),#96，192，384，768
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),#56，28，14，7
                                                 patches_resolution[1] // (2 ** i_layer)),#56，28，14，7
                               depth=depths[i_layer],#[8,8,8,8]
                               num_heads=num_heads[i_layer],#[8,8,8,8]
                               ...
                               norm_layer=norm_layer,
                               downsample=PatchMerging if (i_layer < self.num_layers - 1) else None,#块融合
                               use_checkpoint=use_checkpoint)
            self.layers.append(layer)#4个basiclayer块，每个块里面由8个tra块组成

        # build decoder layers
        self.layers_up = nn.ModuleList()
        self.concat_back_dim = nn.ModuleList()
        for i_layer in range(self.num_layers):#4
            concat_linear = nn.Linear(2 * int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),#（768，384， 192 ，96）*2
                                      int(embed_dim * 2 ** (
                                              self.num_layers - 1 - i_layer))) if i_layer > 0 else nn.Identity()#768，384， 192 ，96
            if i_layer == 0:
              #上采样，本文采用的是双上采样
                layer_up = UpSample(input_resolution=patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),#7，14，28，56
                                    in_channels=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)), scale_factor=2)#768，384， 192 ，96
            else:
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         ...
                                         use_checkpoint=use_checkpoint)
            self.layers_up.append(layer_up)#第一次循环的时候将上采样操作给加入上采样层网络中，后面的循环将BasicLayer_up层加入解码层
            self.concat_back_dim.append(concat_linear)

        self.norm = norm_layer(self.num_features)#768
        self.norm_up = norm_layer(self.embed_dim)#96
        ...
        if self.final_upsample == "Dual up-sample":
            ...
            self.output = nn.Conv2d(in_channels=embed_dim, out_channels=self.out_chans, kernel_size=3, stride=1,
                                    padding=1, bias=False)  # kernel = 1 #返回和原图大小一样的尺寸
    ...
    def forward_features(self, x):
        residual = x#这里传入的x为经过一个3*3卷积的特征图，但是和原图保持了相同的大小 通道数变成了96,可以将输入的3*3卷积换成1*1的卷积，将通道数变成96
    ...
    def forward(self, x):
        x = self.conv_first(x)#此处得到的是3*3卷积得到的特征图
